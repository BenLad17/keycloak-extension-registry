	...
	<div class="mb-8 flex gap-2 pt-2">
		<div class="relative flex-1">
			...
			<input
				...
				onkeydown={(e) => e.key === 'Enter' && goto(`/explore?q=${searchQuery}&category=${selectedCategory}`)}
		...
		<div class="rounded-xl border border-border bg-surface-muted py-16 text-center">
			...
			{:else}
				<p class="text-sm text-text-secondary/60">
					Be the first to <a href="/publish" class="text-brand-fg hover:underline">publish an extension</a>.
				</p>
			{/if}
		</div>
	{:else}
		<div class="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
			{#each data.extensions as { extension: ext, githubOwner, githubRepo }}
				<a
					...
					class="group flex flex-col rounded-xl border border-border bg-surface px-5 py-4 text-text no-underline transition-colors hover:bg-surface-muted"
				>
					<div class="mb-2 flex items-start justify-between gap-3">
						<h3 class="text-sm font-medium leading-snug transition-colors group-hover:text-brand-fg">
							{ext.name}
						</h3>
		...
		{#if data.totalPages > 1}
			<div class="mt-8 flex justify-center gap-1.5">
				{#each Array(data.totalPages) as _, i}
					<a
						href={`/explore?page=${i + 1}&q=${data.query}&category=${data.category}`}
